        ...
        for (const entry of currentSchedule) { // need to re-add indefinitely delayed series to timetables, or correctly remove un-verified episodes.
            ...
            if (((existingInAiring === -1) || (new Date(timetables[existingInAiring].delayedFrom) < new Date() && new Date(timetables[existingInAiring].delayedFrom).getUTCFullYear() > 1 && new Date(timetables[existingInAiring].delayedUntil).getUTCFullYear() <= 1)) && (existingInAiring !== -1 || entry.verified) && entry.episodeNumber < (entry.episodes || Infinity)) { // highly likely this is an indefinitely delayed series.
                if ((existingInAiring !== -1) && !entry.delayedIndefinitely) {
                    ...
                    timetables[existingInAiring] = {
                        ...entry,
                        delayedUntil: (entry.delayedIndefinitely && entry.delayedUntil) || new Date(new Date().getFullYear() + 6, 0, 1).toISOString(),
                        delayedIndefinitely: true
                    }
                }
...
export async function updateDubFeed(optSchedule) {
    ...
    schedule.forEach(entry => {
        ...
        if (latestEpisodeInFeed && !dayTimeMatch(new Date(latestEpisodeInFeed.episode.airedAt), new Date(entry.episodeDate)) && (!entry.subtractedEpisodeNumber || (entry.subtractedEpisodeNumber > 1 && !((entry.episodeNumber - entry.subtractedEpisodeNumber) >= 6)))) {
            ...
            if (((entry.episodeNumber < 4 && !isInDSTTransition) || (!isInDSTTransition && daysAgo(new Date(latestEpisodeInFeed.episode.airedAt), new Date(entry.episodeDate)) <= 8)) && (entry.delayedFrom !== entry.episodeDate || daysAgo(new Date(latestEpisodeInFeed.episode.airedAt), new Date(entry.delayedFrom)) >= 7)) {
    ...
    const newEpisodes = (await Promise.all(schedule.map(async (entry) => {
        ...
        const minEpisode = entry.media?.media?.zeroEpisode ? 0 : 1
        let episodeType = 0
